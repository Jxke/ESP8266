...
void loop() {
 ...
 if (current[0] != last[0] && current[0] == false){
    secret = random(1,100);
    if (secret > 70 && secret != 0){
      votes[0] = votes[0] + 1;//record button1
      secret = 0;
    }
    if (secret < 71 && secret != 0) {
      votes[1] = votes[1] + 1;//record button2
      secret = 0;
    }
  }
  if (current[1] != last[1] && current[1] == false){
    votes[1] = votes[1] + 1;//record button2
  }
  ...
  String message = "Left: " + String(votes[0]) + ", Right: " + String(votes[1]);
